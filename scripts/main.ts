...
function handleEditClick(event: Event, eventsPlanning: any, page: string) {
...
function addEventListenerOnce(elementId : string, eventName : string, handler : EventListener){
...
function validateInput(fieldId : string) : boolean{
    ...
    const errorElement = document.getElementById(`${fieldId}-error`) as HTMLElement | null;
...
// Assuming eventsPlanning is the object holding the current event details
        (document.getElementById("eventName") as HTMLInputElement).value = eventsPlanning.eventName;
        (document.getElementById("location") as HTMLInputElement).value = eventsPlanning.location;
        (document.getElementById("description") as HTMLInputElement).value = eventsPlanning.description;
...
function DisplayEventsListPage() {
    ...
    const eventsList: HTMLElement | null = document.getElementById("eventsList");
    if (!eventsList) {
    ...
    keys.forEach((key: string) => {
        if (key.startsWith("events_")) {
            const eventsListData: string | null = localStorage.getItem(key);
            if (!eventsListData) {
...
function DisplayEventsPlanningPage() {
    ...
    const subscribeCheckbox = document.getElementById("subscribeCheckbox") as HTMLInputElement;
    const eventsListButton = document.getElementById("showEventsList") as HTMLInputElement;
...
export function handleLogout(event: any) {
    ...
    function DisplayLoginPage() {
        ...
        const messageArea = document.getElementById("messageArea") as HTMLButtonElement | null;
...
function handlePageLogic(path: string) {
    ...
    document.addEventListener("routeLoaded", async (event: any) => {
        if (event instanceof CustomEvent) {
            const newPath = event.detail as string;
            console.log(`[INFO] Route Loaded: ${newPath}`);
            updateActiveNewLink();
            CheckLogin();
            initializeFilters();
    ...
    function addDonateLink(): void {
        ...
        donateLink.addEventListener('click', (e: Event) => {
            e.preventDefault();
    ...
    function DisplayTime(): void {
        ...
        const timeDataElement: HTMLElement | null = document.getElementById("time-data");

        if (!timeDataElement) return;
    ...
    function initializeTimeDisplay(): void {
        DisplayTime();
...
// Define the toggleNearbyPlaces function and attach it to the window object
    window.toggleNearbyPlaces = function (): void {
        const placeContainer: HTMLElement | null = document.getElementById("place-container");
        const toggleButton: HTMLElement | null = document.getElementById("toggle-button");
        ...
        findNearbyPlaces().catch((error: Error) => {
     ...
     */
    function findNearbyPlaces(): Promise<void> {
        // API key
        const apiKey: string = "fsq3kSvyOGdNRJ3teXgvsoi8KiBgAymNN8h5zftEnzwDqLA=";
        const placeContainer: HTMLElement | null = document.getElementById("place-container");
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            navigator.geolocation.getCurrentPosition(
                (position: GeolocationPosition) => {
                    ...
                    const url: string = `https://api.foursquare.com/v3/places/nearby?ll=${lat},${lon}&radius=1000&limit=5`;
...
// Attach the searchSite function to the window object
    window.searchSite = function (event: Event): void {
        event.preventDefault();
...
// Event and filter
    function initializeFilters(): void {
        ...
        // Get filter elements
        const eventFilter = document.getElementById("eventFilter") as HTMLSelectElement | null;
        const eventLocation = document.getElementById("eventLocation") as HTMLSelectElement | null;
        ...
        function applyFilter(): void {
            ...
            const eventCards = document.querySelectorAll<HTMLElement>(".col-md-4");
    ...
    // Gallery
        ...
        function initializeGallery(): void {
            ...
            const lightboxText = document.getElementById("lightbox-text") as HTMLElement | null;
